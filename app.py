...
def like_tweet(message_id):
    ...
    if message.user_id != g.user.id:
        g.user.liked_messages.appned(message)
        db.session.commit()

    return redirect(request.referrer or '/')

@app.route('/unlike/<int:message_id>', methods = ['POST'])
def unlike_tweet(message_id):
    if not g.user:
        flash('Invalid Credentials', 'error')
        return redirect('/')
...
def profile():
    if not g.user:
        flash('Invalid username', 'invalid')
        return redirect('/')
    ...
    if form.validate_on_submit():
        if User.authenticate(username=g.user.username, password=form.password.data):
            ...
            flash("Password incorrect, could not update profile.", "danger")
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        flash('Inavlid Credentials, Try again')
